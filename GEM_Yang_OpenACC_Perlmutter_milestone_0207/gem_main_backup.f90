...
!     call balloon
  ...
  do  timestep=ncurr,nm
     tcurr = tcurr+dt
     if((iput.eq.1).and.mod(timestep,500).eq.0)call restart(2,timestep-1)
...
!$acc loop seq
     ...
     if(x3(ns,m)<lx/20.0 .or. x3(ns,m)>(lx-lx/20.))then
!        w3(ns,m) = 0.
!        w2(ns,m) = 0.
     end if
  ...
  if(iflag.eq.1) then
     ...
     do ns = 1,nsm
        ...
        do m=1,mm(ns)
           ...
           read(139+MyId) xii(ns,m),z0i(ns,m),pzi(ns,m),eki(ns,m),u0i(ns,m),g0i(ns,m)
           w2(ns,m)=w2(ns,m)/cut
           x3(ns,m)=x2(ns,m)
           y3(ns,m)=y2(ns,m)
           z3(ns,m)=z2(ns,m)
  ...
  if(iflag.eq.2) then
     ...
     do ns = 1,nsm
        ...
        do m=1,mm(ns)
           ...
           write(139+MyId) xii(ns,m),z0i(ns,m),pzi(ns,m),eki(ns,m),u0i(ns,m),g0i(ns,m)
        enddo
     end do
...
15   format(1x,f10.1,8(2x,e10.3))
     ...
     write(9,10)tdum,rmsphi(n),rmsapa(n),pf,efe,pfi,efi,avewi(1,n),avewi(2,n),&
          avewe(n),yyre(0,0),yyim(0,0),yyamp(0,0)
...
subroutine loadi(ns)
  ...
  do j = 1,100000000
     ...
     if(ran2(iseed)<(0.5*jacp/jacmax))then
        ...
        xii(ns,m) = x2(ns,m)
        u0i(ns,m) = u2(ns,m)
        g0i(ns,m) = 1./(2*pi*ter)**1.5*exp(-eki(ns,m)/ter) *mims(ns)**1.5
...
!$omp private(i,k,ie,r,th,wx0,wx1,wz0,wz1,b,ter,xnp,vfac,xt,ptch,isitin,iptch) 
  do m=1,mm(ns)
     ...
     ptch = u3(ns,m)/sqrt(2*vfac*ter/mims(ns))
     isitin=0
...
!$omp atomic
     myg(i,ie) = myg(i,ie)+1
!$omp atomic
     mynclambda(iptch,gclr) = mynclambda(iptch,gclr)+isitin /jac(int(x3(ns,m)/dx),0)
...
! pull-back of ion weight 
  ...
  do m=1,mm(ns)
     ...
     w3(ns,m) = w3(ns,m) - q(ns)*aparhp*u3(ns,m)/ter*xnp  ! Eq. (15)
     w2(ns,m) = w3(ns,m)
   enddo
